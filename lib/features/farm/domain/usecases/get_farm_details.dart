...
class GetFarmDetails implements UseCase<FarmDetailsResult, GetFarmDetailsParams> {
  final FarmRepository repository;

  const GetFarmDetails(this.repository);

  @override
  Future<Either<Failure, FarmDetailsResult>> call(GetFarmDetailsParams params) async {
    try {
      final farmResult = await repository.getFarm(params.farmId);
      if (farmResult.isLeft()) {
        return farmResult.fold((failure) => Left(failure), (_) => throw Exception());
      }

      final plotsResult = await repository.getFarmPlots(params.farmId);
      if (plotsResult.isLeft()) {
        return plotsResult.fold((failure) => Left(failure), (_) => throw Exception());
      }

      final bedsResult = await repository.getFarmBeds(params.farmId);
      if (bedsResult.isLeft()) {
        return bedsResult.fold((failure) => Left(failure), (_) => throw Exception());
      }

      final plantingsResult = await repository.getFarmPlantings(params.farmId);
      if (plantingsResult.isLeft()) {
        return plantingsResult.fold((failure) => Left(failure), (_) => throw Exception());
      }

      final statsResult = await repository.getFarmStats(params.farmId);
      if (statsResult.isLeft()) {
        return statsResult.fold((failure) => Left(failure), (_) => throw Exception());
      }
      ...
      return Right(FarmDetailsResult(
        ...
        plantings: plantingsResult.fold((_) => throw Exception(), (plantings) => plantings),
